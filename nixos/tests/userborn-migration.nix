...
# safely switched to userborn without UID/GID reuse problems and without losing
# /etc/sub{u,g}id allocations.
#
...
#   auto-subuid-map            name -> subuid range start
#
# See nixos/modules/services/system/userborn-import-legacy.py and
# pkgs/by-name/ni/nixos-subids/ for the implementations under test.

{
  ...
  nodes.machine = {
    ...
    users.users = {
      ...
      podman-user = {
        isNormalUser = true;
        # autoSubUidGidRange defaults to true for normal users without explicit
        # ranges; spell it out for clarity.
  ...
  testScript =
    ...
    ''
      ...
      with subtest("perl: capture allocated state"):
          ...
          subuid_map = json.loads(machine.succeed("cat /var/lib/nixos/auto-subuid-map"))
          t.assertIn("podman-user", subuid_map)
          podman_subuid = subuid_map["podman-user"]

          podman_subuid_line = machine.succeed("grep '^podman-user:' /etc/subuid").strip()
          t.assertEqual(podman_subuid_line, f"podman-user:{podman_subuid}:65536")

          explicit_subuid_line = machine.succeed("grep '^explicit:' /etc/subuid").strip()
          t.assertEqual(explicit_subuid_line, "explicit:300000:65536")
      ...
      with subtest("userborn: subid continuity"):
          machine.succeed("systemctl is-active nixos-subids.service")

          # newuidmap (shadow-utils) opens /etc/subuid with O_NOFOLLOW.
          ...
          t.assertEqual(
              podman_subuid_line,
              f"podman-user:{podman_subuid}:65536",
              "podman-user lost its auto-allocated subuid range across migration",
          )
      ...
      with subtest("idempotency"):
          ...
          t.assertEqual(before, after, "nixos-subids is not idempotent")
    '';
}
