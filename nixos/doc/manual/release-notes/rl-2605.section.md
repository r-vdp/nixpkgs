...
- Enabling [`services.userborn`](#opt-services.userborn.enable) on a system
  that was previously managed by the default `update-users-groups.pl` script
  now imports the legacy state from `/var/lib/nixos/` on the first switch.
  Locked stub entries are added to `/etc/passwd` and `/etc/group` for every
  name recorded in `uid-map`/`gid-map` that no longer has a live entry, so a
  previously-used UID/GID cannot be reassigned to a different user.
...
- Explicitly configured [`users.users.<name>.subUidRanges`](#opt-users.users._name_.subUidRanges)
  and [`subGidRanges`](#opt-users.users._name_.subGidRanges) that overlap
  across users now produce an evaluation warning. Set
  [`users.subIdRanges.strictOverlapCheck`](#opt-users.subIdRanges.strictOverlapCheck)
  to additionally refuse writing `/etc/sub{u,g}id` at runtime when any
  overlap (including against existing auto-allocated ranges) is detected.
