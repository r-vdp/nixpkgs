...
- [services.gnome.gnome-keyring](#opt-services.gnome.gnome-keyring.enable) now starts the daemon via systemd socket activation as the user unit `gnome-keyring-daemon.service`, instead of letting PAM/XDG-autostart spawn it ad-hoc. The `cap_ipc_lock` security wrapper has been dropped: it was bypassed by the PAM-spawned daemon in practice and hard-fails under `no_new_privs`; the daemon's `mlock()` of secret memory is now ensured via `LimitMEMLOCK=` on the unit instead. The `gnome-keyring` package no longer rewrites its autostart/D-Bus entries to `/run/wrappers/bin` by default; pass `useWrappedDaemon = true` to restore the old behaviour.
