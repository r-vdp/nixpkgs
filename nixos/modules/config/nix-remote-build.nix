...
let
  ...
  buildMachines =
    if isAttrs cfg.buildMachines
    then attrValues cfg.buildMachines
    else
      warn
        "defining nix remote builders as a list is deprecated, please use an attrset instead"
        cfg.buildMachines;
    ...
    nix = {
      buildMachines = mkOption {
        ...
        type =
          ...
          in
          types.either
            (types.listOf buildMachinesOpts)
            (types.attrsOf buildMachinesOpts);
        default = { };
        description = ''
      ...
      [
        {
          ...
          (concatStringsSep "\n      "
            (map (m: m.hostName)
              (filter badMachine buildMachines)));
        }
      ];
