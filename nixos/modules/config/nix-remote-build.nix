...
let
  ...
  # TODO: remove the list type after 24.11 got released
  buildMachines =
    if isAttrs cfg.buildMachines then
      attrValues cfg.buildMachines
    else
      warn "defining nix remote builders as a list is deprecated, please use an attrset instead" cfg.buildMachines;
...
in
{
  options = {
    nix = {
      buildMachines = mkOption {
        ...
        type =
          let
            buildMachinesOpts = types.submodule {
              options = {
                hostName = mkOption {
                  type = types.str;
          ...
          in
          types.either (types.listOf buildMachinesOpts) (types.attrsOf buildMachinesOpts);
        default = { };
        description = ''
  ...
  config = mkIf cfg.enable {
    assertions =
      ...
      [
        {
          assertion = !(any badMachine buildMachines);
          message = ''
          ...
          ''
          + "      "
          + (concatStringsSep "\n      " (map (m: m.hostName) (filter badMachine buildMachines)));
        }
      ];

    # List of machines for distributed Nix builds
    environment.etc."nix/machines" = mkIf (buildMachines != { }) {
